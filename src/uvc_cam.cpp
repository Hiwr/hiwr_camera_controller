...
{
    /// opening device
    ROS_INFO("[Uvc Cam] opening video device");
    // printf("opening %s\n", _device);
    ...
    if (fmt.fmt.pix.width != width || fmt.fmt.pix.height != height){
        char errorMesage[50];
        sprintf(errorMesage , "pixel format unavailable %d - %d  vs %d %d \n",fmt.fmt.pix.width ,fmt.fmt.pix.height, width, height ); 
    ...
    if ((ret = ioctl(fd, VIDIOC_S_PARM, &streamparm)) < 0){
        throw std::runtime_error("unable to set framerate");}else{
      printf("frame rate correctly setted\n");
  }
    ...
    printf("Will try to set some stufff \n");
    try 
    {
        ...
        set_control(V4L2_CID_CONTRAST, 200);
        //   printf("set contrast\n");
        set_control(V4L2_CID_WHITE_BALANCE_TEMP_AUTO_OLD, 0);
        // auto white balance
        set_control(0x98090c, 0);

        //set_control(0x98091c , 0); //back light compensation
    ...
    printf("DID try to set some stufff \n");
    /*
...
int Cam::grab(unsigned char **frame, uint32_t* bytes_used_p)
{
    *frame = NULL;
    int ret = 0;
    fd_set rdset;
    ...
    try { 
        ...
        *bytes_used_p = bytes_used;
        ...
        printf("catch exception during last part  \n");
        throw e;
    }
    return buf.index;
}
...
{
    printf("RELEASE STATUS OF FD is %d\n",fcntl(fd, F_GETFD));
